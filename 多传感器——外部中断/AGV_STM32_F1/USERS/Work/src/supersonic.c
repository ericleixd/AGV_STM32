...
u8 ultroCount = 0x06;//程序运行时，可以获取到的超声波数量
...
void data_send_pc(u16* dis_buf)
{
	...
	for(int i =0; i<6;i++)
	{
		ROS_USB_USART_Send_byte(dis_buf[i]>>8 &0xFF);
	
	ROS_USB_USART_Send_byte(dis_buf[i] &0xFF);
	}
